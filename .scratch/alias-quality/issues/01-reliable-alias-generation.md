...
- `PATCH /admin/docs/{id}` 端到端实跑（2026-09-19 补测，此前只是构造性修复）：
  插一条合成 doc → 真实 HTTP PATCH 返回 `200 {"ok":true}`（修复前这条路径必 500）→
  正文回读已更新、`knowledge_history` 记下旧文、无 token 返回 401 →
  别名按新正文重建 5 条且 `parent_text` 全部指向改写后文本 → 删除自检 doc 与其别名、
  清掉 changelog 两行审计记录，集合回到 986、缺别名 0，服务日志 0 条 error。
  全程未触碰任何真实知识。
